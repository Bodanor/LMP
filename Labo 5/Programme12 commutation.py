...
def mettre_a_jour_moteur(t):
    global angle_moteur, vitesse_angulaire, courant_enroulement
    K = 1000
    R = 0.02
    L = 0.06
    B = 0.5
    ...
    if ((angle_moteur >= 0 and angle_moteur < (math.pi / 2)) or ((angle_moteur > (3 * math.pi / 2)) and (angle_moteur < (2 * math.pi)))):
    ...
    Phi = 2 * K * R * L * courant_enroulement * B * math.cos(angle_moteur)
